...
def makeDWR(o_host='192.0.2.11', o_realm='example.com'):
...
def makeDWA(resultCode=2001, o_host='192.0.2.11', o_realm='example.com'):
...
def makeCER(o_host='192.0.2.11', o_realm='example.com', prod_name='AiDiameter'):
...
def makeCCR(sessionId="csdk;hlapi;1611836847258625",
            ...
            origHost='client-host.example.com',
            origRealm='example.com',
            destRealm='example2.com',
...
def makeDPR(o_host='192.0.2.11', o_realm='example.com'):
    m = Message(cmdflags=MessageFlags.REQUEST.value, cmdcode=280, appId=0)
    originHost = AVP(code=264, flags=0x40, data=DiameterIdentity(o_host))
    originRealm = AVP(code=296, flags=0x40, data=DiameterIdentity(o_realm))
    avps = [originHost, originRealm]
...
def makeDPA(o_host='192.0.2.11', o_realm='example.com'):
    m = Message(cmdflags=MessageFlags.REQUEST.value, cmdcode=280, appId=0)
    originHost = AVP(code=264, flags=0x40, data=DiameterIdentity(o_host))
    originRealm = AVP(code=296, flags=0x40, data=DiameterIdentity(o_realm))
    avps = [originHost, originRealm]
