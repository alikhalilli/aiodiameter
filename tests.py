...
if __name__ == "__main__":
    m = Message(cmdflags=flags['Request'],
                ...
                appId=0)
    originHost = AVP(
        code=264, flags=avpflags['mandatory'], data=DiameterIdentity('192.0.2.11'))
    originRealm = AVP(
        code=296, flags=avpflags['mandatory'], data=DiameterIdentity('example.com'))
    host_IP_Address = AVP(
        code=257, flags=avpflags['mandatory'], data=Address('192.0.2.11'))
    vendorID = AVP(
