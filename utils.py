...
def makeCER():
    ...
    originHost = baseavp.AVP(code=264, flags=0x40,
                             data=diamidentity.DiameterIdentity('192.0.2.11'))
    originRealm = baseavp.AVP(code=296, flags=64,
                              data=diamidentity.DiameterIdentity('example.com'))
    host_IP_Address = baseavp.AVP(code=257, flags=0x40,
                                  data=address.Address('192.0.2.11'))
...
def makeCEA():
    ...
    originHost = AVP(code=264, flags=0x40, data=DiameterIdentity('192.0.2.11'))
    originRealm = AVP(code=296, flags=0x40,
                      data=DiameterIdentity('example.com'))
    host_IP_Address = AVP(code=257, flags=0x40, data=Address('192.0.2.11'))
...
def makeDWR():
    ...
    originHost = AVP(
        code=264, flags=0x40, data=DiameterIdentity('192.0.2.11'))
    originRealm = AVP(
        code=296, flags=0x40, data=DiameterIdentity('example.com'))
...
def makeCCR():
    ...
    destinationRealm = baseavp.AVP(code=283,
                                   flags=0x40,
                                   data=diamidentity.DiameterIdentity('example2.com'))
    ...
    subscriptionID = baseavp.AVP(code=443, flags=0xc0, vendorID=193, data=groupedAVP.GroupedAVP([
        baseavp.AVP(code=444, flags=0xc0, vendorID=193,
                    data=utf8string.UTF8String('994500000000')),
